...
const STR = {
  np: {
    ...
    phoneLabel: 'फोन वा इमेल',
    passLabel: 'पासवर्ड',
    loginBtn: 'लग-इन गर्नुहोस्',
    guestBtn: 'पाहुनाको रूपमा जारी राख्नुहोस्',
    loginNote: 'यो डेमो लग-इन हो — कुनै पासवर्ड पनि चल्छ (कम्तीमा ४ अक्षर)।',
    loginErr: 'कृपया नाम, फोन/इमेल र कम्तीमा ४ अक्षरको पासवर्ड हाल्नुहोस्।',
    // tabs
    tabHome: 'गृह',
    tabCheck: 'जाँच',
    tabChat: 'सहयोगी',
  ...
  en: {
    ...
    loginNote: 'This is a demo login — any password works (min 4 characters).',
    loginErr: 'Please enter your name, phone/email and a password of at least 4 characters.',
    tabHome: 'Home',
    tabCheck: 'Check',
    tabChat: 'Helper',
    tabLearn: 'Learn',
...
function Login({ lang, t, onLogin }) {
  ...
  const submit = () => {
    if (!name.trim() || !phone.trim() || pass.trim().length < 4) {
      setErr(true);
      return;
    }
    onLogin(name.trim());
    ...
    <SafeAreaView style={[styles.fill, { backgroundColor: C.bg }]}>
      ...
      <KeyboardAvoidingView style={styles.fill} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.loginWrap} keyboardShouldPersistTaps="handled">
          ...
          <Text style={styles.loginSub}>{t.loginSub}</Text>

          <View style={[styles.loginCard, SHADOW]}>
            <Text style={styles.inputLabel}>{t.nameLabel}</Text>
            <TextInput style={styles.input} placeholder={t.nameLabel} placeholderTextColor={C.muted} value={name} onChangeText={(v) => { setName(v); setErr(false); }} />

            <Text style={styles.inputLabel}>{t.phoneLabel}</Text>
            <TextInput style={styles.input} placeholder={t.phoneLabel} placeholderTextColor={C.muted} value={phone} onChangeText={(v) => { setPhone(v); setErr(false); }} keyboardType="email-address" autoCapitalize="none" />

            <Text style={styles.inputLabel}>{t.passLabel}</Text>
            <TextInput style={styles.input} placeholder={t.passLabel} placeholderTextColor={C.muted} value={pass} onChangeText={(v) => { setPass(v); setErr(false); }} secureTextEntry />

            {err && <Text style={styles.loginErr}>{t.loginErr}</Text>}

            <TouchableOpacity style={[styles.primaryBtn, { marginTop: 6 }]} onPress={submit} activeOpacity={0.85}>
              <Text style={styles.primaryBtnText}>{t.loginBtn}</Text>
            </TouchableOpacity>

            <TouchableOpacity style={styles.guestBtn} onPress={() => onLogin('')} activeOpacity={0.7}>
...
function ReportScreen({ t }) {
  ...
  const submit = () => {
    if (!name.trim()) return;
    setSeed((s) => [{ name, what: what || '—', level: 'danger', ago: 'now' }, ...s]);
  ...
  return (
    <KeyboardAvoidingView style={styles.fill} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.scrollPad} keyboardShouldPersistTaps="handled">
        ...
        {seed.map((r, i) => (
          <View key={i} style={[styles.reportItem, SHADOW]}>
            <View style={{ flex: 1 }}>
            ...
            <View style={{ alignItems: 'flex-end' }}>
              ...
              <Text style={styles.reportAgo}>{r.ago}</Text>
            </View>
          </View>
        ))}
...
export default function App() {
  ...
  if (screen === 'welcome') {
    return (
      <>
        ...
        <Welcome lang={lang} setLang={setLang} onDone={() => setScreen('login')} />
      </>
    );
  }
  ...
  if (screen === 'login') {
    return (
      <Login
        lang={lang}
        t={t}
        onLogin={(name) => { setUserName(name); setScreen('app'); }}
      />
    );
  }
  ...
  return (
    <SafeAreaView style={[styles.fill, { backgroundColor: C.bg }]}>
      ...
      {/* Top app bar with language switch */}
...
const styles = StyleSheet.create({
  ...
  guestBtn: { paddingVertical: 14, alignItems: 'center', marginTop: 6 },
